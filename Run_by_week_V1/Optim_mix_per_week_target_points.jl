...
# On identifie les indices du dernier jour de la semaine (heures 145 à 168)
# On s'arrête juste avant l'anneau de garde
last_day_indices1 = (168-23):168 
last_day_indices2 = (84-23):84

for t in last_day_indices1
    # Calcul de la moyenne pour le Nord et le Sud (qui partagent les mêmes niveaux relatifs)
    avg_target = (high_lev_CH4[t] + low_lev_CH4[t]) / 2
    
    # On écrase les valeurs pour que le min et le max soient identiques
    high_lev_CH4[t] = avg_target
    low_lev_CH4[t] = avg_target
end

for t in last_day_indices2
    # Calcul de la moyenne pour le Nord et le Sud (qui partagent les mêmes niveaux relatifs)
    avg_target = (high_lev_CH4[t] + low_lev_CH4[t]) / 2
