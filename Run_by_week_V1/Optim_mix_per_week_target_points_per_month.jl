...
if !isnothing(idx_premier_du_mois)
    t_target = idx_premier_du_mois
    
    avg_target = (high_lev_CH4[t_target] + low_lev_CH4[t_target]) / 2
    
    # Application de la contrainte "point cible" (min = max = moyenne)
    high_lev_CH4[t_target] = avg_target
    low_lev_CH4[t_target] = avg_target
...
mois_trimestriels = [1, 4, 7, 10]

# On cherche si le 1er jour d'un mois cible se trouve dans la semaine actuelle (heures 1 à 168)
idx_cible = findfirst(d -> Dates.day(d) == 1 && Dates.month(d) in mois_trimestriels, dates[1:168])

if !isnothing(idx_cible)
    t_target = idx_cible
    
    # Calcul de la moyenne cible pour ce pas de temps spécifique
    avg_target = (high_lev_CH4[t_target] + low_lev_CH4[t_target]) / 2
    
    # Application de la contrainte stricte (point cible)
    high_lev_CH4[t_target] = avg_target
    low_lev_CH4[t_target] = avg_target
    
    println(">>> POINT CIBLE TRIMESTRIEL détecté à l'index $t_target ($(dates[t_target]))")
end
