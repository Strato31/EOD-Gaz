...
if target_idx !== nothing
    # Stockage cible à 88 % le 1O octobre
    @constraint(model, stock_CH4_S[target_idx] == 0.88 * stock_CH4_max_S)
    @constraint(model, stock_CH4_N[target_idx] == 0.88 * stock_CH4_max_N)
end
...
# CH4 injections / sorties
write(f, "CH4_S_inj ; CH4_S_sout ; CH4_N_inj ; CH4_N_sout ; ")

# CH4 stocks
write(f, "CH4_S_stock ; CH4_N_stock ; ")

# CH4 flux
write(f, "CH4_flux_S_N ; ")

# H2 production / flux
write(f, "H2_S_prod ; H2_N_prod ; H2_flux_S_N ; ")

# Unsupplied energy
write(f, "CH4_S_uns ; CH4_N_uns ; H2_S_uns ; H2_N_uns ; ")
...
for t in 1:Tmax
    ...
    # CH4 injections / sorties
    write(f, "$(P_inj_CH4_S_val[t]) ; $(P_sout_CH4_S_val[t]) ; " *
               "$(P_inj_CH4_N_val[t]) ; $(P_sout_CH4_N_val[t]) ; ")

    # CH4 stocks
    write(f, "$(stock_CH4_S_val[t]) ; $(stock_CH4_N_val[t]) ; ")

    # CH4 flux
    write(f, "$(flux_CH4_val[t]) ; ")

    # H2 production / flux
    write(f, "$(P_H2_S_val[t]) ; $(P_H2_N_val[t]) ; $(flux_H2_val[t]) ; ")
    ...
    # Binary states UC
    for g in 1:Nth
        write(f, "$(value(UCth[t,g])) ; ")
    end
    for g in 1:N_CH4
        write(f, "$(value(UC_CH4_S[t,g])) ; $(value(UC_CH4_N[t,g])) ; ")
    end
    write(f, "$(value(UC_H2_S[t])) ; $(value(UC_H2_N[t])) \n")
end
...
write(f, "stock_STEP;$(value(stock_STEP[Tmax]))\n")
write(f, "stock_battery;$(value(stock_battery[Tmax]))\n")
write(f, "stock_CH4_S;$(value(stock_CH4_S[Tmax]))\n")
write(f, "stock_CH4_N;$(value(stock_CH4_N[Tmax]))\n")

# ---- Binary states fin de semaine ----
for g in 1:Nth
    write(f, "UCth_$g;$(value(UCth[Tmax,g]))\n")
end
for g in 1:N_CH4
    write(f, "UC_CH4_S_$g;$(value(UC_CH4_S[Tmax,g]))\n")
    write(f, "UC_CH4_N_$g;$(value(UC_CH4_N[Tmax,g]))\n")
end
write(f, "UC_H2_S;$(value(UC_H2_S[Tmax]))\n")
write(f, "UC_H2_N;$(value(UC_H2_N[Tmax]))\n")
