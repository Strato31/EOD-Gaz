...
if week_id == 0
   ...
   #rename!(df, name -> strip(String(name)))
   ...
   stock_CH4_N_init   = df[end, "stock_CH4_N"]
    ...
    upth_cols = ["UPth_$g" for g in 1:Nth]
    do_thcols = ["DOth_$g" for g in 1:Nth]
    UPth_init = Matrix{Int}(df[:, upth_cols])
    DOth_init = Matrix{Int}(df[:, do_thcols])

    upch4n_cols = ["UP_CH4_N_$g" for g in 1:N_CH4]
    upch4s_cols = ["UP_CH4_S_$g" for g in 1:N_CH4]
    doch4n_cols = ["DO_CH4_N_$g" for g in 1:N_CH4]
    doch4s_cols = ["DO_CH4_S_$g" for g in 1:N_CH4]
    UP_CH4_N_init = Matrix{Int}(df[:, upch4n_cols])
    UP_CH4_S_init = Matrix{Int}(df[:, upch4s_cols])
    DO_CH4_N_init = Matrix{Int}(df[:, doch4n_cols])
    DO_CH4_S_init = Matrix{Int}(df[:, doch4s_cols])

    UP_H2_N_init = reshape(Int.(df[:, "UP_H2_N"]), :, 1)   # matrice n×1
    UP_H2_S_init = reshape(Int.(df[:, "UP_H2_S"]), :, 1)
    DO_H2_N_init = reshape(Int.(df[:, "DO_H2_N"]), :, 1)
    DO_H2_S_init = reshape(Int.(df[:, "DO_H2_S"]), :, 1)
...
for g in 1:Nth
    if init_UC 
        ...
        @constraint(model, [t in 1:dmin[g]-1], UCth[t,g] >= sum(UPth[i,g] for i in 1:t))
        @constraint(model, [t in 1:dmin[g]-1], UCth[t,g] <= 1 - (sum(DOth[i,g] for i in 1:t)))
    end
...
for g in 1:N_CH4
    if init_UC 
        ...
        @constraint(model, [t in 1:dmin_CH4_N[g]-1], UC_CH4_S[t,g] >= sum(UP_CH4_S[i,g] for i in 1:t) + sum(UP_CH4_S_init[T_end-i+1,g] for i in 1:dmin_CH4_S[g]-t))
        @constraint(model, [t in 1:dmin_CH4_N[g]-1], UC_CH4_S[t,g] <= 1 - (sum(DO_CH4_S[i,g] for i in 1:t) + sum(DO_CH4_S_init[T_end-i+1,g] for i in 1:dmin_CH4_S[g]-t)))
    else
        @constraint(model, [t in 1:dmin_CH4_N[g]-1], UC_CH4_N[t,g] >= sum(UP_CH4_N[i,g] for i in 1:t))
        @constraint(model, [t in 1:dmin_CH4_N[g]-1], UC_CH4_N[t,g] <= 1 - (sum(DO_CH4_N[i,g] for i in 1:t)))
        @constraint(model, [t in 1:dmin_CH4_S[g]-1], UC_CH4_N[t,g] >= sum(UP_CH4_S[i,g] for i in 1:t))
        @constraint(model, [t in 1:dmin_CH4_S[g]-1], UC_CH4_N[t,g] <= 1 - (sum(DO_CH4_S[i,g] for i in 1:t)))
    end
...
if init_UC 
    ...
    @constraint(model, [t in 1:dmin_H2_N-1], UC_H2_S[t] >= sum(UP_H2_S[i] for i in 1:t) + sum(UP_H2_S_init[T_end-i+1] for i in 1:dmin_H2_S-t))
    @constraint(model, [t in 1:dmin_H2_N-1], UC_H2_S[t] <= 1 - (sum(DO_H2_S[i] for i in 1:t) + sum(DO_H2_S_init[T_end-i+1] for i in 1:dmin_H2_S-t)))
else
    ...
    @constraint(model, [t in 1:dmin_H2_N-1], UC_H2_S[t] >= sum(UP_H2_S[i] for i in 1:t))
    @constraint(model, [t in 1:dmin_H2_N-1], UC_H2_S[t] <= 1 - (sum(DO_H2_S[i] for i in 1:t)))
end
...
for g in 1:N_CH4
    write(f, "UP_CH4_S_$g;UP_CH4_N_$g;")
    write(f, "DO_CH4_S_$g;DO_CH4_N_$g;")
end

write(f, "UP_H2_S;UP_H2_N;DO_H2_S;DO_H2_N\n")
...
for t in 1:Tmax
    ...
    sum_Puns_CH4_S = sum(Puns_CH4_S_val[t,g] for g in 1:N_CH4)
    sum_Puns_CH4_N = sum(Puns_CH4_N_val[t,g] for g in 1:N_CH4)
    ...
    for g in 1:Nth
        write(f, "$(Int(value(DOth[t,g])));")
        write(f, "$(Int(value(UCth[t,g])));")
    end
    for g in 1:N_CH4
        write(f, "$(Int(value(UP_CH4_S[t,g])));$(Int(value(UP_CH4_N[t,g])));")
        write(f, "$(Int(value(DO_CH4_S[t,g])));$(Int(value(DO_CH4_N[t,g])));")
    end

    write(f, "$(Int(value(UP_H2_S[t])));$(Int(value(UP_H2_N[t])));" *
    "$(Int(value(DO_H2_S[t])));$(Int(value(DO_H2_N[t])))\n")
