...
length_anneau_garde_days = 6 #jours
...
avg_lev_CH4 = (high_lev_CH4 + low_lev_CH4) ./ 2
...
@variable(model, dev_CH4_S_pos[1:Tmax] >= 0) # Écart au dessus de la moyenne
@variable(model, dev_CH4_S_neg[1:Tmax] >= 0) # Écart en dessous de la moyenne
@variable(model, dev_CH4_N_pos[1:Tmax] >= 0)
@variable(model, dev_CH4_N_neg[1:Tmax] >= 0)
...
@objective(model, Min,
  ...
  + c_dev_CH4 * ( sum(dev_CH4_S_pos[t] + dev_CH4_S_neg[t] for t in 1:Tmax) + sum(dev_CH4_N_pos[t] + dev_CH4_N_neg[t] for t in 1:Tmax) )
)
...
# Contrainte de lien : Stock = Cible + déviation_pos - déviation_neg
@constraint(model, target_link_S[t in 1:Tmax], 
    stock_CH4_S[t] == (avg_lev_CH4[t] * stock_CH4_max_S) + dev_CH4_S_pos[t] - dev_CH4_S_neg[t]
)

@constraint(model, target_link_N[t in 1:Tmax], 
    stock_CH4_N[t] == (avg_lev_CH4[t] * stock_CH4_max_N) + dev_CH4_N_pos[t] - dev_CH4_N_neg[t]
)
