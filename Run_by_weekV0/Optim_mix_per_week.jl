...
if week_id == 0
    ...
    stock_CH4_S_init = 0.0
    stock_CH4_N_init = 0.0
    init_UC = false
else
    ...
    UC_CH4_S_init = [state["UC_CH4_S_$g"] for g in 1:N_CH4]
    UC_CH4_N_init = [state["UC_CH4_N_$g"] for g in 1:N_CH4]
    UC_H2_S_init = state["UC_H2_S"]
    UC_H2_N_init = state["UC_H2_N"]
    UCth_init = [state["UCth_$g"] for g in 1:Nth]
    init_UC = true
end
...
if init_UC
    for g in 1:Nth
        @constraint(model, UCth[1,g] == UCth_init[g])
    end
    for g in 1:N_CH4
        @constraint(model, UC_CH4_S[1,g] == UC_CH4_S_init[g])
        @constraint(model, UC_CH4_N[1,g] == UC_CH4_N_init[g])
    end
    @constraint(model, UC_H2_S[1] == UC_H2_S_init)
    @constraint(model, UC_H2_N[1] == UC_H2_N_init)
end
