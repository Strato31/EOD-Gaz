...
def check_min_down_time(series, dmin, unit_name):
    """
    Vérifie que chaque séquence de production > 0
    dure au moins dmin heures.
    """
...
# ==========================
# TEST DES DUREES MINIMALES DES MOYENS DE PRODUCTION
# ==========================
...
for i, name in enumerate(th_names):
    ...
    violations += check_min_down_time(
        df[name],
        int(dmin_th[i]),
        f"Thermal_{name}"
    )
...
# ==========================
# VERIFICATION DU NIVEAU DE STOCK CH4 LE 01/10
# ==========================

stock_CH4_N = df.loc["2020-10-01", "stock_CH4_N"]
stock_CH4_S = df.loc["2020-10-01", "stock_CH4_S"]

stock_CH4 = stock_CH4_N + stock_CH4_S

stock_CH4_pct = stock_CH4 / (20*10**6)

stock_CH4_pct_val = stock_CH4_pct.values
time = stock_CH4_pct.index
n = len(stock_CH4_pct_val)

is_at_088 = True
error_margin = 0.005 #0,5%
t=0

while is_at_088 and t<n :
    if (stock_CH4_pct_val[t] < 0.88-error_margin) or (stock_CH4_pct_val[t] > 0.88+error_margin):
        is_at_088 = False
        print(f"La contrainte de stock CH4 à 88% le 1er octobre n'est pas respectée à partir de l'heure {t} : stock CH4 = {stock_CH4_pct_val[t]*100:.2f} %")
    else :
        t+=1

if is_at_088:
    print("La contrainte de stock CH4 à 88% le 1er octobre est respectée.")
